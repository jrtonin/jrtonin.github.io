...
GLOc = BASE1 %>%
  ...
  group_by(market, covid_wave, variable) %>%
  summarise(ttest = list(t.test(lider, seguidora))) %>%
...
mutate(id = sequence(n()))
  group_by(market, covid_wave) %>%
  summarise(ttest_price = list(t.test(price ~ position, paired = TRUE)),
            ttest_aircraft = list(t.test(aircraft ~ position, paired = TRUE))) %>%
  mutate(ttest_price = map(ttest_price, tidy),
         ttest_aircraft = map(ttest_aircraft, tidy)) %>%
  unnest()


  mutate(across(c("estimate", "p.value", "estimate1", "p.value1"), ~ round(.x, 3))) %>%
  mutate(id = case_when(covid_wave == "normal" ~ 1,
                        covid_wave == "first" ~ 2, TRUE ~ 3)) %>%
  arrange(market, id) %>%
  mutate(GLO_price = paste0(estimate, case_when(p.value <= 0.01 ~ "***",
                                                p.value > 0.01 & p.value <=0.05 ~ "**",
                                                p.value > 0.05 & p.value <=0.1 ~ "*", 
                                                TRUE ~ "")),
         GLO_aircraft = paste0(estimate1, case_when(p.value1 <= 0.01 ~ "***",
                                                p.value1 > 0.01 & p.value1 <=0.05 ~ "**",
                                                p.value1 > 0.05 & p.value1 <=0.1 ~ "*", 
                                                TRUE ~ ""))) %>%
  select(market, covid_wave, GLO_price, GLO_aircraft)

} # organizing GOL dabase

TESTE = BASE1 %>%
  filter(N >= 2 & airline %in% c("GLO", "AZU", "TAM")) %>%
  group_by(airline, covid_wave, market) %>%
  dplyr::summarise(price = sum(revenue)/sum(seat_dem),
                   aircraft = sum(seat_offer)/sum(take_off)) %>%
  gather(variable, value, price:aircraft) %>%
  spread(market, value) %>%
  mutate(diff = lider-seguidora) %>%
  mutate(id = case_when(covid_wave == "normal" ~ 1,
                        covid_wave == "first" ~ 2, TRUE ~ 3)) %>%
  arrange(id)
